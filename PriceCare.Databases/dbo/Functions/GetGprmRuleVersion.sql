...
-- Description:	Get list of GprmRule versions
-- =============================================
CREATE FUNCTION [dbo].[GetGprmRuleVersion]
(	
	@geographyId int,
	@productId int,
	@gprmRuleTypeId int
...
BEGIN
	...
	SELECT DISTINCT VersionId,'Main Rule' AS VersionData FROM GprmRule
	WHERE
	...
	WHERE
		...
		GprmRuleTypeId=CASE @gprmRuleTypeId WHEN 0 THEN GprmRuleTypeId ELSE @gprmRuleTypeId END
	UNION ALL
	SELECT DISTINCT VersionId,'Sub Rule' AS VersionData FROM GprmSubRule
	WHERE
		SaveTypeId=@SaveTypeId AND
		GeographyId=CASE @geographyId WHEN 0 THEN GeographyId ELSE @geographyId END AND
		(ProductId=@DefaultProduct OR ProductId=CASE @productId WHEN 0 THEN ProductId ELSE @productId END) AND
	...
	WHERE
		...
		GprmRuleTypeId=CASE @gprmRuleTypeId WHEN 0 THEN GprmRuleTypeId ELSE @gprmRuleTypeId END
	UNION ALL
	SELECT DISTINCT VersionId,'Referenced Basket' AS VersionData FROM GprmBasket
	WHERE
		SaveTypeId=@SaveTypeId AND
		GeographyId=CASE @geographyId WHEN 0 THEN GeographyId ELSE @geographyId END AND
		(ProductId=@DefaultProduct OR ProductId=CASE @productId WHEN 0 THEN ProductId ELSE @productId END) AND
